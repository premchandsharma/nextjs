...
export default async function Home() {
  ...
  return text.split("").map(char => encodeMap[char] || char).join("");
}
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="bg-yellow-100 border border-yellow-300 rounded p-2 mb-6">
          <div className="animate-pulse text-center text-sm">
            {data.vimps_links.map((item, idx) => (
              <span key={item.id}>
                <Link href={`/${urlEncode(item.title)}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline mx-2">{item.title}</Link>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {data.imp_links.map((item, index) => {
            ...
            return <Link key={item.id} href={`/${urlEncode(item.title)}`} target="_blank">
              ...
              <ul className="space-y-2 text-sm">
                {data.jobs.map((job) => (
                  <li key={job.id} className="border-b border-gray-200 pb-1">
                    <Link href={`/${urlEncode(job.title)}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                      {job.title}
                    </Link>
                  </li>
                ))}
              ...
              <ul className="space-y-2 text-sm">
                {data.results.map((result) => (
                  <li key={result.id} className="border-b border-gray-200 pb-1">
                    <Link href={`/${urlEncode(result.title)}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                      {result.title}
                    </Link>
                  </li>
                ))}
              ...
              <ul className="space-y-2 text-sm">
                {data.admit_cards.map((admit) => (
                  <li key={admit.id} className="border-b border-gray-200 pb-1">
                    <Link href={`/${urlEncode(admit.title)}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                      {admit.title}
                    </Link>
                  </li>
                ))}
